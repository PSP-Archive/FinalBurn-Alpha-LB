...
#define MAX_TIMER_VALUE (0x7FFF0000)

float dTime;									// Time elapsed since the emulated machine was started

static int ALIGN_DATA nTimerCount[2], nTimerStart[2];

// Callbacks
static UINT8 (*pTimerOverCallback)(UINT8, UINT8);
...
static float BurnTimerTimeCallbackDummy()
{
	return 0.0;
}
...
UINT8 BurnTimerUpdate(int nCycles)
{
	UINT8 nIRQStatus = 0;
...
void BurnOPLTimerCallback(int c, float period)
{
	pCPURunEnd();
...
//		bprintf(PRINT_NORMAL, _T("  - timer %i stopped\n"), c);
	...
	nTimerCount[c]  = (int)(period * (float)TIMER_TICKS_PER_SECOND);
...
void BurnOPMTimerCallback(int c, float period)
{
	pCPURunEnd();
...
void BurnOPNTimerCallback(int  /*n */, int c, int cnt, float stepTime)
{
	pCPURunEnd();
...
//		bprintf(PRINT_NORMAL, _T("  - timer %i stopped\n"), c);
	...
	nTimerCount[c]  = (int)(stepTime * cnt * (float)TIMER_TICKS_PER_SECOND);
	nTimerCount[c] += MAKE_TIMER_TICKS(pCPUTotalCycles(), nCPUClockspeed);
//	bprintf(PRINT_NORMAL, _T("  - timer %i started, %08X ticks (fires in %lf seconds)\n"), c, nTimerCount[c], stepTime * cnt);
}

void BurnYMFTimerCallback(int /* n */, int c, float period)
{
	pCPURunEnd();
...
//		bprintf(PRINT_NORMAL, _T("  - timer %i stopped\n"), c);
	...
	nTimerStart[c]  = nTimerCount[c] = (int)(period * (float)(TIMER_TICKS_PER_SECOND / 1000000));
	nTimerCount[c] += MAKE_TIMER_TICKS(pCPUTotalCycles(), nCPUClockspeed);

//	bprintf(PRINT_NORMAL, _T("  - timer %i started, %08X ticks (fires in %lf seconds)\n"), c, nTimerCount[c], period);
}

void BurnTimerSetRetrig(int c, float period)
{
	pCPURunEnd();
...
//		bprintf(PRINT_NORMAL, _T("  - timer %i stopped\n"), c);
	...
	nTimerStart[c]  = nTimerCount[c] = (int)(period * (float)(TIMER_TICKS_PER_SECOND));
	nTimerCount[c] += MAKE_TIMER_TICKS(pCPUTotalCycles(), nCPUClockspeed);

//	bprintf(PRINT_NORMAL, _T("  - timer %i started, %08X ticks (fires in %lf seconds)\n"), c, nTimerCount[c], period);
}

void BurnTimerSetOneshot(int c, float period)
{
	pCPURunEnd();
...
int BurnTimerInit(UINT8 (*pOverCallback)(UINT8, UINT8), float (*pTimeCallback)())
{
	BurnTimerExit();
