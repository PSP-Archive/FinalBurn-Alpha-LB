...
static struct QChan ALIGN_DATA QChan[16];

static int ALIGN_DATA PanningVolumes[33];
...
int QscInit(int nRate, int nVolumeShift)
{
	...
	for (int i = 0; i < 33; i++) {
		PanningVolumes[i] = (int)((256.0 / sqrtf(32.0)) * sqrtf((float)i));
	}
...
inline static void QscSyncQsnd()
{
...
#else
//	BurnSoundCopyClamp_Mips(Qs_s, pBurnSoundOut + (nPos << 1), nLen);
	BurnSoundCopyClamp_C(Qs_s, pBurnSoundOut + (nPos << 1), nLen);
#endif
	nPos = nEnd;
