...
int ALIGN_DATA nRasterline[MAX_RASTER + 2];

int nCpsLcReg = 0;						// Address of layer controller register
int ALIGN_DATA CpsLayEn[6] = {0, 0, 0, 0, 0, 0};	// bits for layer enable
int ALIGN_DATA MaskAddr[4] = {0, 0, 0, 0};
...
#ifdef BUILD_PSP
				PutPix(pBurnDraw + (nStarYPos << 10) + (nStarXPos << 1), CpsPal[0x0800 + (nLayer << 9) + nStarColour]);
#else
