...
inline static float MegadriveGetTime()
{
	return (float)ZetTotalCycles() / (OSC_NTSC / 15);
}
...
//	}
	...
	if( Hardware & 0x40 ) { // 
		//cycles_68k = (int) ((float) OSC_PAL  /  7 / 50 / 312 + 0.4); // should compile to a constant (488)
		//cycles_z80 = (int) ((float) OSC_PAL  / 15 / 50 / 312 + 0.4); // 228
		lines  = 312;    // Steve Snake says there are 313 lines, but this seems to also work well
		line_sample = 68;
		if( RamVReg->reg[1]&8 ) lines_vis = 240;
	} else {
		//cycles_68k = (int) ((float) OSC_NTSC /  7 / 60 / 262 + 0.4); // 488
		//cycles_z80 = (int) ((float) OSC_NTSC / 15 / 60 / 262 + 0.4); // 228
		lines  = 262;
		line_sample = 93;
	}
