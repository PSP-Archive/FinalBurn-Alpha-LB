...
UINT16 ALIGN_DATA TC0100SCNCtrl[TC0100SCN_MAX_CHIPS][8];
...
void TC0100SCNRenderBgLayer(int Opaque, unsigned char *pSrc)
{
	...
	for (my = 0; my < 64; my++) {
		for (mx = 0; mx < Columns; mx++) {
			...
			Code = (VideoRam[Offset + 1] & TC0100SCNGfxMask) + (TC0100SCNGfxBank << 15);
			Colour = Attr & 0xff;
			Flip = (Attr & 0xc000) >> 14;
			xFlip = (Flip >> 0) & 0x01;
			yFlip = (Flip >> 1) & 0x01;
			...
			for (py = 0; py < 8; py++) {
				for (px = 0; px < 8; px++) {
					unsigned char c = pSrc[(Code << 6) + (py << 3) + px];
					if (xFlip) c = pSrc[(Code << 6) + (py << 3) + (7 - px)];
					if (yFlip) c = pSrc[(Code << 6) + ((7 - py) << 3) + px];
					if (xFlip && yFlip) c = pSrc[(Code << 6) + ((7 - py) << 3) + (7 - px)];
					...
					if (c || Opaque) {
						...
						if (yPos >= 0 && yPos < nScreenHeight) {					
							if (!TC0100SCNDblWidth) {							
								if (!TC0100SCNFlip && !TC0100SCNFlipScreenX) xPos -= ((BgScrollX - BgScrollRam[yPos + TC0100SCNYOffset]) & 0x1ff);
								if (TC0100SCNFlip || TC0100SCNFlipScreenX) xPos += ((BgScrollX + BgScrollRam[yPos + TC0100SCNYOffset]) & 0x1ff);
						
								if (xPos < -8) xPos += 512;
								if (xPos >= 512) xPos -= 512;
							} else {
								if (!TC0100SCNFlip && !TC0100SCNFlipScreenX) xPos -= ((BgScrollX - BgScrollRam[yPos + TC0100SCNYOffset]) & 0x3ff);
								if (TC0100SCNFlip || TC0100SCNFlipScreenX) xPos += ((BgScrollX + BgScrollRam[yPos + TC0100SCNYOffset]) & 0x3ff);
			...
			for (py = 0; py < 8; py++) {
				for (px = 0; px < 8; px++) {
					...
					if (c || Opaque) {
						int xPos = x + px;
						int yPos = y + py;
						if (!TC0100SCNFlip) yPos -= (FgScrollY - FgColScrollRam[mx]) & 0x1ff;
						if (TC0100SCNFlip) yPos += (FgScrollY + FgColScrollRam[mx]) & 0x1ff;
						...
						if (yPos >= 0 && yPos < nScreenHeight) {
							if (!TC0100SCNDblWidth) {					
								if (!TC0100SCNFlip && !TC0100SCNFlipScreenX) xPos -= ((FgScrollX - FgScrollRam[yPos + TC0100SCNYOffset]) & 0x1ff);
								if (TC0100SCNFlip || TC0100SCNFlipScreenX) xPos += ((FgScrollX + FgScrollRam[yPos + TC0100SCNYOffset]) & 0x1ff);
						
								if (xPos < -8) xPos += 512;
								if (xPos >= 512) xPos -= 512;
							} else {
								if (!TC0100SCNFlip && !TC0100SCNFlipScreenX) xPos -= ((FgScrollX - FgScrollRam[yPos + TC0100SCNYOffset]) & 0x3ff);
								if (TC0100SCNFlip || TC0100SCNFlipScreenX) xPos += ((FgScrollX + FgScrollRam[yPos + TC0100SCNYOffset]) & 0x3ff);
							...
							if (xPos >= 0 && xPos < nScreenWidth) {
								pPixel[xPos] = c | nPalette;
								if (TC0100SCNPriorityMap) TC0100SCNPriorityMap[(yPos * nScreenWidth) + xPos] = 2;
							}
						}
					}
				}
...
static int ALIGN_DATA TC0100SCNPlaneOffsets[2] = { 8, 0 };
static int ALIGN_DATA TC0100SCNXOffsets[8]     = { 0, 1, 2, 3, 4, 5, 6, 7 };
static int ALIGN_DATA TC0100SCNYOffsets[8]     = { 0, 16, 32, 48, 64, 80, 96, 112 };
...
void TC0100SCNRenderCharLayer()
{
	...
	for (my = 0; my < Rows; my++) {
		for (mx = 0; mx < Columns; mx++) {
			...
			for (py = 0; py < 8; py++) {
				for (px = 0; px < 8; px++) {
					unsigned char c = TC0100SCNChars[(Code << 6) + (py << 3) + px];
					if (xFlip) c = TC0100SCNChars[(Code << 6) + (py << 3) + (7 - px)];
					if (yFlip) c = TC0100SCNChars[(Code << 6) + ((7 - py) << 3) + px];
					if (xFlip && yFlip) c = TC0100SCNChars[(Code << 6) + ((7 - py) << 3) + (7 - px)];
