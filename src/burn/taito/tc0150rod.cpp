...
void TC0150RODDraw(int yOffs, int pOffs, int Type, int RoadTrans, int LowPriority, int HighPriority)
{
	...
	UINT16 ALIGN_DATA ScanLine[512];
	UINT16 ALIGN_DATA RoadALine[512];
	UINT16 ALIGN_DATA RoadBLine[512];
	UINT16 *Dst16;
	UINT16 *RoadA;
	UINT16 *RoadB;
	...
	int RoadAAddress = (yOffs << 2) + ((RoadCtrl & 0x0300) << 2);
	int RoadBAddress = (yOffs << 2) + ((RoadCtrl & 0x0c00) << 0);
	
	int PrioritySwitchLine = (RoadCtrl & 0x00ff) - yOffs;

	do {
		LineNeedsDrawing = 0;
		RoadRamIndex  = RoadAAddress + (y << 2);
		RoadRam2Index = RoadBAddress + (y << 2);
		...
		if (BackgroundOnly) {
			...
			if ((LeftEdge >= 0) && (LeftEdge < nScreenWidth)) {
				...
				if (LineNeedsDrawing) {
					for (i = LeftEdge; i >= 0; i--)	{
						GfxWord = RoadGfx[(RoadGfxTileNum << 8) + (xIndex >> 3)];
						Pixel = (((GfxWord >> (7 - (xIndex % 8) + 8)) & 1) << 1) + ((GfxWord >> (7 - (xIndex % 8))) & 1);
