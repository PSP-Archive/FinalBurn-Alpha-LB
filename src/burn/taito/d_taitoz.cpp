...
static struct BurnRomInfo BsharkRomDesc[] = {
	...
	{ "c34_67.64",     0x20000, 0x39307c74, BRF_ESS | BRF_PRG | TAITO_68KROM1_BYTESWAP },
...
{
	TC0100SCNCtrlWordWrite_Map(0xa20000)
...
static UINT8 BsharkStickRead(int Offset)
{
	switch (Offset) {
		case 0x00: {
			int Temp = (TaitoAnalogPort0 >> 4) & 0xfff;
			Temp = 0xfff - Temp;
			Temp += 1;
...
{
	TC0220IOCHalfWordWrite_Map(0x400000)
	TC0100SCNCtrlWordWrite_Map(0xd20000)
...
{
	TC0100SCNCtrlWordWrite_Map(0xc20000)
...
static UINT8 ContcircInputBypassRead()
{
	...
	int Steer = (TaitoAnalogPort0 >> 4) & 0xfff;
	Steer = 0xfff - Steer;
...
{
	TC0100SCNCtrlWordWrite_Map(0x220000)
...
static UINT16 DblaxleSteerRead(int Offset)
{
...
unsigned char __fastcall Dblaxle68K1ReadByte(unsigned int a)
{
...
void __fastcall Dblaxle68K1WriteByte(unsigned int a, unsigned char d)
{
...
unsigned short __fastcall Dblaxle68K1ReadWord(unsigned int a)
{
	TC0510NIOHalfWordSwapRead_Map(0x400000)
...
void __fastcall Dblaxle68K1WriteWord(unsigned int a, unsigned short d)
{
	TC0510NIOHalfWordSwapWrite_Map(0x400000)
...
static const UINT8 ALIGN_DATA nightstr_stick[128]=
{
...
static UINT8 NightstrStickRead(int Offset)
{
	switch (Offset) {
		case 0x00: {
			unsigned char Temp = 0x7f + (TaitoAnalogPort0 >> 4);
		...
		case 0x01: {
			unsigned char Temp = 0x7f - (TaitoAnalogPort1 >> 4);
			if (Temp < 0x01) Temp = 0x01;
			if (Temp > 0xfe) Temp = 0xfe;
			return nightstr_stick[(Temp * 0x64) / 0x100];
		}
...
static UINT8 ALIGN_DATA spacegun_default_eeprom[128]=
{
...
static int ALIGN_DATA DblaxleCharPlaneOffsets[4] = { 0, 1, 2, 3 };
static int ALIGN_DATA DblaxleCharXOffsets[16]    = { 4, 0, 20, 16, 12, 8, 28, 24, 36, 32, 52, 48, 44, 40, 60, 56 };
static int ALIGN_DATA DblaxleCharYOffsets[16]    = { 0, 64, 128, 192, 256, 320, 384, 448, 512, 576, 640, 704, 768, 832, 896, 960 };
static int ALIGN_DATA SpritePlaneOffsets[4]      = { 0, 8, 16, 24 };
static int ALIGN_DATA SpriteXOffsets[16]         = { 32, 33, 34, 35, 36, 37, 38, 39, 0, 1, 2, 3, 4, 5, 6, 7 };
static int ALIGN_DATA SpriteYOffsets[16]         = { 0, 64, 128, 192, 256, 320, 384, 448, 512, 576, 640, 704, 768, 832, 896, 960 };
static int ALIGN_DATA Sprite16x8YOffsets[8]      = { 0, 64, 128, 192, 256, 320, 384, 448 };
...
static float TaitoZGetTime()
{
	return (float)ZetTotalCycles() / (16000000 / 4);
}

static int TaitoZ68KSynchroniseStream(int nSoundRate)
{
	return (long long)SekTotalCycles() * nSoundRate / (nTaitoCyclesTotal[1] * 60);
}

static float TaitoZ68KGetTime()
{
	return (float)SekTotalCycles() / (nTaitoCyclesTotal[1] * 60);
}
...
static int AquajackInit()
{
	...
	SekMapMemory(TC0100SCNRam[0]         , 0xa00000, 0xa0ffff, SM_RAM);
	...
	BurnYM2610Init(16000000 / 2, 25.0, 100.0, TaitoYM2610ARom, (int*)&TaitoYM2610ARomSize, TaitoYM2610BRom, (int*)&TaitoYM2610BRomSize, &TaitoZFMIRQHandler, TaitoZSynchroniseStream, TaitoZGetTime, 0);
...
static int BsharkInit()
{
	...
	TC0100SCNInit(1, TaitoNumChar, 0, 8, 1, NULL);
	TC0220IOCInit();
	...
	SekMapMemory(TC0100SCNRam[0]         , 0xd00000, 0xd0ffff, SM_RAM);
	...
	BurnYM2610Init(16000000 / 2, 25.0, 100.0, TaitoYM2610ARom, (int*)&TaitoYM2610ARomSize, TaitoYM2610BRom, (int*)&TaitoYM2610BRomSize, NULL, TaitoZ68KSynchroniseStream, TaitoZ68KGetTime, 0);
...
static int ChasehqInit()
{
	...
	SekMapMemory(TC0100SCNRam[0]         , 0xc00000, 0xc0ffff, SM_RAM);
...
static int ContcircInit()
{
	...
	TC0110PCRInit(1, 0x1000);
	TC0150RODInit(TaitoRoadRomSize, 0);
	TC0100SCNInit(1, TaitoNumChar, 0, 16, 0, NULL);
	TC0140SYTInit();
	TC0220IOCInit();
	...
	SekMapMemory(TC0100SCNRam[0]         , 0x200000, 0x20ffff, SM_RAM);
...
static int SpacegunInit()
{
	...
	TC0110PCRInit(1, 0x1000);
	TC0100SCNInit(1, TaitoNumChar, -4, 8, 1, NULL);
	TC0220IOCInit();
...
static void BsharkRenderSprites(int PriorityDraw, int yOffset, int SpriteRamSize)
{
	UINT16 *SpriteMap = (UINT16*)TaitoSpriteMapRom;
	UINT16 *SpriteRam = (UINT16*)TaitoSpriteRam;
	int Offset, Data, Tile, Colour, xFlip, yFlip;
...
static void AquajackDraw()
{
	...
	if (TC0100SCNBottomLayer()) {
		if (!(Disable & 0x02)) TC0100SCNRenderFgLayer(1, TaitoChars);
		if (!(Disable & 0x01)) TC0100SCNRenderBgLayer(0, TaitoChars);
	} else {
		if (!(Disable & 0x01)) TC0100SCNRenderBgLayer(1, TaitoChars);
...
static int TaitoZFrame()
{
	...
	for (int i = 1; i <= nInterleave; i++) {
		...
		if (i == TaitoFrameInterleave) SekSetIRQLine(TaitoIrqLine, SEK_IRQSTATUS_AUTO);
		SekClose();
