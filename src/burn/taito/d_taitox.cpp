...
static struct BurnDIPInfo SuprmanjDIPList[]=
{
	// Default Values
	{0x13, 0xff, 0xff, 0xff, NULL                             },
	{0x14, 0xff, 0xff, 0xff, NULL                             },
...
static struct BurnDIPInfo TwinhwkuDIPList[]=
{
	// Default Values
	{0x15, 0xff, 0xff, 0xff, NULL                             },
	{0x16, 0xff, 0xff, 0x7f, NULL                             },
...
static struct BurnRomInfo GigandsjRomDesc[] = {
...
static struct BurnRomInfo TwinhawkRomDesc[] = {
	{ "b87-11.bin",    0x20000, 0xfc84a399, BRF_ESS | BRF_PRG | TAITO_68KROM1_BYTESWAP },
	{ "b87-10.bin",    0x20000, 0x17181706, BRF_ESS | BRF_PRG | TAITO_68KROM1_BYTESWAP },

	{ "b87-07",        0x08000, 0xe2e0efa0, BRF_ESS | BRF_PRG | TAITO_Z80ROM1 },

	{ "b87-02",        0x80000, 0x89ad43a0, BRF_GRA | TAITO_SPRITESA },
	{ "b87-01",        0x80000, 0x81e82ae1, BRF_GRA | TAITO_SPRITESA },
	{ "b87-04",        0x80000, 0x958434b6, BRF_GRA | TAITO_SPRITESA },	
	{ "b87-03",        0x80000, 0xce155ae0, BRF_GRA | TAITO_SPRITESA },
...
static struct BurnRomInfo TwinhwkuRomDesc[] = {
	{ "b87-09.u6",     0x20000, 0x7e6267c7, BRF_ESS | BRF_PRG | TAITO_68KROM1_BYTESWAP },
	{ "b87-08.u4",     0x20000, 0x31d9916f, BRF_ESS | BRF_PRG | TAITO_68KROM1_BYTESWAP },
...
static struct BurnRomInfo DaisenpuRomDesc[] = {
	{ "b87-06",        0x20000, 0xcf236100, BRF_ESS | BRF_PRG | TAITO_68KROM1_BYTESWAP },
	{ "b87-05",        0x20000, 0x7f15edc7, BRF_ESS | BRF_PRG | TAITO_68KROM1_BYTESWAP },
...
static int MemIndex()
{
	...
	TaitoPalette                    = (unsigned int*)Next; Next += 0x00800 * sizeof(unsigned int);
...
unsigned char __fastcall TaitoX68KReadByte(unsigned int a)
{
...
void __fastcall TaitoX68KWriteByte(unsigned int a, unsigned char d)
{
	if (TaitoIC_SupermanCChipInUse) {
		if (a >= 0x900000 && a <= 0x9007ff) {
...
unsigned short __fastcall TaitoX68KReadWord(unsigned int a)
{
	switch (a) {
		case 0x500000: {
			return TaitoDip[0] & 0x0f;
...
unsigned char __fastcall TaitoXZ80Read(unsigned short a)
{
	switch (a) {
		case 0xe000: {
...
void __fastcall TaitoXZ80Write(unsigned short a, unsigned char d)
{
	switch (a) {
		case 0xe000: {
			BurnYM2610Write(0, d);
...
static void TaitoXFMIRQHandler(int, int nStatus)
{
	if (nStatus & 1) {
...
static int TaitoXSynchroniseStream(int nSoundRate)
{
	return (long long)ZetTotalCycles() * nSoundRate / 4000000;
}

static float TaitoXGetTime()
{
	return (float)ZetTotalCycles() / 4000000;
}

static void TaitoXYM2151IRQHandler(int Irq)
{
	if (Irq) {
		ZetSetIRQLine(0xff, ZET_IRQSTATUS_ACK);
	} else {
...
static int ALIGN_DATA SpritePlaneOffsets[4]   = { 0x800008, 0x800000, 8, 0 };
static int ALIGN_DATA SpriteXOffsets[16]      = { 0, 1, 2, 3, 4, 5, 6, 7, 128, 129, 130, 131, 132, 133, 134, 135 };
static int ALIGN_DATA SpriteYOffsets[16]      = { 0, 16, 32, 48, 64, 80, 96, 112, 256, 272, 288, 304, 320, 336, 352, 368 };
static int ALIGN_DATA BallbrosPlaneOffsets[4] = { 0x300000, 0x200000, 0x100000, 0 };
static int ALIGN_DATA BallbrosXOffsets[16]    = { 0, 1, 2, 3, 4, 5, 6, 7, 64, 65, 66, 67, 68, 69, 70, 71 };
static int ALIGN_DATA BallbrosYOffsets[16]    = { 0, 8, 16, 24, 32, 40, 48, 56, 128, 136, 144, 152, 160, 168, 176, 184 };

static int TaitoXInit(int bYM2151)
{
	int nLen;
	...
	if (bYM2151) {
		...
		BurnYM2610Init(8000000, 25.0, 100.0, TaitoYM2610BRom, (int*)&TaitoYM2610BRomSize, TaitoYM2610ARom, (int*)&TaitoYM2610ARomSize, &TaitoXFMIRQHandler, TaitoXSynchroniseStream, TaitoXGetTime, 0);
...
static void TaitoXDrawBgSprites()
{
	...
	int Offs, Col, x, y, Code, Colour, xFlip, yFlip, sx, sy, yOffs;

	int Ctrl = SpriteRam[0x300];
	int Ctrl2 = SpriteRam[0x301];
	...
	int Upper = (SpriteRam[0x302] & 0xff) +	(SpriteRam[0x303] & 0xff) * 256;
	int Col0;
	switch (Ctrl & 0x0f) {
...
static void TaitoXDrawSprites()
{
	...
	int Offset, Code, x, y, xFlip, yFlip, Colour;
	int Ctrl = SpriteRam[0x300];
	int Ctrl2 = SpriteRam[0x301];
	int Flip = Ctrl & 0x40;
	UINT16 *src = SpriteRam2 + (((Ctrl2 ^ (~Ctrl2 << 1)) & 0x40) ? 0x1000 : 0);
...
static int TwinhawkFrame()
{
	...
	for (int i = 1; i <= nInterleave; i++) {
		...
		if (i == nInterleave) SekSetIRQLine(2, SEK_IRQSTATUS_AUTO);
		SekClose();
...
static int TaitoXScan(int nAction, int *pnMin)
{
	struct BurnArea ba;
...
struct BurnDriver BurnDrvGigandsj = {
