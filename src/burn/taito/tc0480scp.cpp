...
UINT16 ALIGN_DATA TC0480SCPCtrl[0x18];
static int ALIGN_DATA BgScrollX[4];
static int ALIGN_DATA BgScrollY[4];
...
static const UINT16 ALIGN_DATA TC0480SCPBgPriLookup[8] =
{
	0x0123,
	0x1230,
	0x2301,
...
static void RenderTile(unsigned short* pDestDraw, int nTileNumber, int StartX, int StartY, int nTilePalette, int nColourDepth, int nPaletteOffset, unsigned char *pTile)
{
	...
	unsigned short* pPixel = pDestDraw + (StartY << 9) + StartX;
...
static void RenderTile_FlipY(unsigned short* pDestDraw, int nTileNumber, int StartX, int StartY, int nTilePalette, int nColourDepth, int nPaletteOffset, unsigned char *pTile)
{
	...
	unsigned short* pPixel = pDestDraw + ((StartY + 15) << 9) + StartX;
...
inline static void DrawScanLine(int y, const UINT16 *src, int Transparent, int /*Pri*/)
{
...
static void TC0480SCPRenderLayer01(int Layer, int Opaque, unsigned char *pSrc)
{
	...
	yZoom = 0x10000 - (((TC0480SCPCtrl[0x08 + Layer] & 0xff) - 0x7f) << 9);
	...
	sx = ((BgScrollX[Layer] + 15 + (Layer << 2)) << 16) + ((255 - (TC0480SCPCtrl[0x10 + Layer] & 0xff)) << 8);
	sx += (-TC0480SCPXOffset - 15 - (Layer << 2)) * xZoom;
...
static void TC0480SCPRenderLayer23(int Layer, int Opaque, unsigned char *pSrc)
{
	...
	do {
		...
		xIndex = sx - ((ScrollRam[RowIndex] << 16)) - ((ScrollRam[RowIndex + 0x800] << 8) & 0xffff);
		xIndex -= (TC0480SCPXOffset - 0x1f + (Layer << 2)) * ((RowZoom & 0xff) << 8);

		xStep = xZoom;
		if (RowZoom) {
			if (!(RowZoom & 0xff00)) {
				xStep -= ((RowZoom << 8) & 0xffff);
			} else {
				xStep -= (((RowZoom & 0xff) << 8) & 0xffff);
			}
		}
...
static int ALIGN_DATA TC0480SCPPlaneOffsets[4] = { 0, 1, 2, 3 };
static int ALIGN_DATA TC0480SCPXOffsets[8]     = { 4, 0, 12, 8, 20, 16, 28, 24 };
static int ALIGN_DATA TC0480SCPYOffsets[8]     = { 0, 32, 64, 96, 128, 160, 192, 224 };
