...
#ifdef FBA															/* !!! FBA */
	void (*timer_callback)(int, int, float);
#endif																/* !!! FBA */
...
static YMF278BChip ALIGN_DATA YMF278B[MAX_YMF278B];
static INT32 ALIGN_DATA volume[256*4];				// precalculated attenuation values with some marging for enveloppe and pan levels
static int ALIGN_DATA pan_left[16], pan_right[16];	// pan volume offsets
static INT32 ALIGN_DATA mix_level[8];
...
static int ymf278b_compute_rate(YMF278BSlot *slot, int val)
{
	...
	if(slot->RC != 15)
	{
		...
		res = ((oct+slot->RC) << 1) + (slot->FN & 0x200 ? 1 : 0) + (val << 2);
	}
	else
		res = val << 2;
	if(res < 0)
...
static void ymf278b_envelope_next(YMF278BSlot *slot, float clock_ratio)
{
	...
	if(slot->env_step == 1)
	{
		...
		if(slot->DL)
		{
			...
			if(rate<4)
				...
				slot->env_vol_step = ((slot->DL)<<26) / (ymf278_compute_decay_rate(rate) * clock_ratio);
			slot->env_vol_lim = (slot->DL)<<26;
			return;
		}
	}
	if(slot->env_step == 2)
	{
		...
		if(rate<4)
			...
			slot->env_vol_step = ((256U-(slot->DL<<3))<<23) / (ymf278_compute_decay_rate(rate) * clock_ratio);
		slot->env_vol_lim = 256U<<23;
		slot->env_step++;
		return;
	}
		...
		if (slot->active)
		{
			...
			for (j = 0; j < length; j++)
			{
				switch (slot->bits)
				{
					...
					case 16:  	// 16 bit
						sample = rombase[slot->startaddr + ((slot->stepptr>>16)<<1)]<<8;
						sample |= rombase[slot->startaddr + ((slot->stepptr>>16)<<1) + 1];
						break;
				}
...
#ifdef FBA															/* !!! FBA */
UINT8 ymf278b_timer_over(UINT8 num, UINT8 timer)
{
	YMF278BChip *chip = &YMF278B[num];
...
static void ymf278b_timer_a_reset(int num)
{
	...
	if(chip->enable & 1)
	{
		float period = (256-chip->timer_a_count) * 80.8 * chip->clock_ratio;
...
static void ymf278b_timer_b_reset(int num)
{
	...
	if(chip->enable & 2)
	{
		float period = (256-chip->timer_b_count) * 323.1 * chip->clock_ratio;
...
#ifdef FBA															/* !!! FBA */
int ymf278b_start(INT8 num, UINT8 *rom, void (*irq_cb)(int, int), void (*timer_cb)(int, int, float), int clock, int rate)
{
	int i;
	...
	for(i = 0; i < 256; i++)
		volume[i] = 65536*powf(2.0, (-0.375/6)*i);
	for(i = 256; i < 256*4; i++)
		volume[i] = 0;
