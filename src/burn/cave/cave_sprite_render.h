...
#if ROT == 0
 #define ADVANCEWORD pPixel += ((BPP >> 3) << 4)
 #ifndef BUILD_PSP
  #define ADVANCEROW pRow += ((BPP >> 3) * XSIZE)
 #else
  #define ADVANCEROW pRow += ((BPP >> 3) << 9)
 #endif 
#else
...
#define TESTCOLOUR(x) x
 ...
 #define PLOTPIXEL(a,b) if (TESTCOLOUR(b) && TESTZBUF(a)) {			\
   	WRITEZBUF(a);													\
	*((UINT16 *)(pPixel + (a << 1))) = (UINT16)pSpritePalette[b];	\
 }
#elif BPP == 24
 #define PLOTPIXEL(a,b) if (TESTCOLOUR(b) && TESTZBUF(a)) {			\
	WRITEZBUF(a);													\
 ...
 #define PLOTPIXEL(a,b) if (TESTCOLOUR(b) && TESTZBUF(a)) {			\
	WRITEZBUF(a);													\
	*((UINT32*)(pPixel + (a << 2))) = (UINT32)pSpritePalette[b];	\
 }
#else
...
// Create an empty function if unsupported features are requested
 ...
 #if ZBUFFER == 0
	for (nSpriteRow = 0; nSpriteRow < nYSize; ADVANCEROW, nSpriteRow++, nSpriteDataOffset += (nSpriteRowSize << 2)) {
 #else
	for (nSpriteRow = 0; nSpriteRow < nYSize; ADVANCEROW, ADVANCEZROW, nSpriteRow++, nSpriteDataOffset += (nSpriteRowSize << 2)) {
 #endif
		nColumn = nXPos;
		UINT32 *pSpriteData = (UINT32 *)getBlock(nSpriteDataOffset, nXSize << 2);
 #if ZBUFFER == 0
  #if XFLIP == 0
