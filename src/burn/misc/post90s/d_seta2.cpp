...
static unsigned char ALIGN_DATA DrvJoy5[8] = {0, 0, 0, 0, 0, 0, 0, 0};
...
inline static unsigned int CalcCol(unsigned short nColour)
{
	...
	return ((nColour & 0x7C00) >> 10) | ((nColour & 0x03E0) << 1) | ((nColour & 0x001F) << 11);
...
static int ALIGN_DATA tmp68301_timer[3] = {0, 0, 0};
static int ALIGN_DATA tmp68301_timer_counter[3] = {0, 0, 0};
static int ALIGN_DATA tmp68301_irq_vector[8] = {0, 0, 0, 0, 0, 0, 0, 0};

static void tmp68301_update_timer( int i )
{
	unsigned short ALIGN_DATA TCR	=	RamTMP68301[(0x200 + i * 0x20)/2];
	unsigned short ALIGN_DATA MAX1	=	RamTMP68301[(0x204 + i * 0x20)/2];
	unsigned short ALIGN_DATA MAX2	=	RamTMP68301[(0x206 + i * 0x20)/2];
...
{
	unsigned short ALIGN_DATA TCR	= RamTMP68301[(0x200 + i * 0x20)/2];
	unsigned short ALIGN_DATA IMR	= RamTMP68301[0x94/2];		// Interrupt Mask Register (IMR)
	unsigned short ALIGN_DATA ICR	= RamTMP68301[0x8e/2+i];	// Interrupt Controller Register (ICR7..9)
	unsigned short ALIGN_DATA IVNR	= RamTMP68301[0x9a/2];		// Interrupt Vector Number Register (IVNR)
