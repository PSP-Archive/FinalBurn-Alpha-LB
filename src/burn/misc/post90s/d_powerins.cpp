...
static unsigned char ALIGN_DATA DrvInput[8] = {0, 0, 0, 0, 0, 0, 0, 0};
...
static float powerinsGetTime()
{
	return (float)ZetTotalCycles() / 6000000;
}
...
static int powerinsInit()
{
	...
	if ( strcmp(BurnDrvGetTextA(DRV_NAME), "powerins") == 0
	  || strcmp(BurnDrvGetTextA(DRV_NAME), "powerinj") == 0) {
	...
	if ( game_drv == GAME_POWERINA ) {
		// M68000 + MSM6295
		MSM6295Init(0, 990000 / 165, 80.0, 0);
	} else {
		if (game_drv == GAME_POWERINS ) {
		// M68000 + Z80 + YM2203 + MSM6295 x 2
			BurnYM2203Init(1, 1500000, 15.0, 15.0, &powerinsIRQHandler, powerinsSynchroniseStream, powerinsGetTime, 0);
			BurnTimerAttachZet(6000000);
			
			MSM6295Init(0, 4000000 / 165, 80.0, 1);
			MSM6295Init(1, 4000000 / 165, 80.0, 1);
		} else {
		// M68000 + Z80 + MSM6295 x 2
			MSM6295Init(0, 4000000 / 165, 80.0, 0);
			MSM6295Init(1, 4000000 / 165, 80.0, 0);
		}
	}
...
//	nCyclesTotal[0] = (int)((long long)12000000 * nBurnCPUSpeedAdjust / (0x0100 * 56));
	nCyclesTotal[0] = (int)((long long)12000000 / 56);
	nCyclesTotal[1] = (int)((long long)6000000  / 56);
...
//	nCyclesTotal[0] = (int)((long long)12000000 * nBurnCPUSpeedAdjust / (0x0100 * 60));
	nCyclesTotal[0] = (int)((long long)12000000 / 60);
	SekRun(nCyclesTotal[0]);
	SekSetIRQLine(4, SEK_IRQSTATUS_AUTO);
	SekClose();
...
//	nCyclesTotal[0] = (int)((long long)12000000 * nBurnCPUSpeedAdjust / (0x0100 * 60));
	nCyclesTotal[0] = (int)((long long)12000000 / 60);
	nCyclesTotal[1] = (int)((long long)6000000  / 60);

	SekOpen(0);
	ZetOpen(0);
	
	SekRun(nCyclesTotal[0]);
	ZetRun(nCyclesTotal[1]);
...
struct BurnDriver BurnDrvPowerina = {
	...
	powerinsInit, powerinsExit, powerinaFrame, NULL, powerinsScan, 0, NULL, NULL, NULL, &bRecalcPalette,
	320, 224, 4, 3
};
...
struct BurnDriver BurnDrvPowerinb = {
	...
	powerinsInit, powerinsExit, powerinbFrame, NULL, powerinsScan, 0, NULL, NULL, NULL, &bRecalcPalette,
	320, 224, 4, 3
};
