...
static unsigned char ALIGN_DATA DrvInput[3]      = {0, 0, 0};
...
static unsigned char *DrvProtData         = NULL;
static unsigned char *DrvMSM6295ROMSrc    = NULL;
static unsigned char *DrvSpriteRam        = NULL;
static unsigned char *DrvPf1Ram           = NULL;
static unsigned char *DrvPf2Ram           = NULL;
...
static int DrvSoundLatch;
static int Tumbleb2MusicCommand;
static int Tumbleb2MusicBank;
static int Tumbleb2MusicIsPlaying;

static int DrvSpriteXOffset;
static int DrvSpriteYOffset;
static int DrvSpriteRamSize;
static int DrvSpriteMask;
static int DrvSpriteColourMask;
static int DrvYM2151Freq;
static int DrvNumSprites;
static int DrvNumChars;
static int DrvNumTiles;
static int DrvHasZ80;
static int DrvHasYM2151;
static int DrvHasYM3812;
static int DrvHasProt;
static int Tumbleb2;
static int Jumpkids;
static int Chokchok;
static int Wlstar;
...
static int Pf1XOffset;
static int Pf1YOffset;
static int Pf2XOffset;
static int Pf2YOffset;

typedef int (*LoadRoms)();
...
static const int Tumbleb2SoundLookup[256] = {
...
{
	int Sound = Tumbleb2SoundLookup[data & 0xff];
...
unsigned char __fastcall Tumbleb68KReadByte(unsigned int a)
{
	switch (a) {
		case 0x100001: {
			return ~0;
...
void __fastcall Tumbleb68KWriteByte(unsigned int a, unsigned char d)
{
	switch (a) {
		case 0x100000: {
			if (Tumbleb2) {
...
unsigned short __fastcall Tumbleb68KReadWord(unsigned int a)
{
	switch (a) {
		case 0x100004: {
			return rand() % 0x10000;
...
void __fastcall Tumbleb68KWriteWord(unsigned int a, unsigned short d)
{
...
unsigned short __fastcall Suprtrio68KReadWord(unsigned int a)
{
	switch (a) {
		case 0xe00000: {
			return ((0xff - DrvInput[1]) << 8) | (0xff - DrvInput[0]);
...
void __fastcall Suprtrio68KWriteWord(unsigned int a, unsigned short d)
{
...
void __fastcall Fncywld68KWriteByte(unsigned int a, unsigned char d)
{
	switch (a) {
		case 0x100001: {
...
unsigned short __fastcall Fncywld68KReadWord(unsigned int a)
{
	switch (a) {
		case 0x180000: {
			return ((0xff - DrvInput[1]) << 8) | (0xff - DrvInput[0]);
...
void __fastcall Fncywld68KWriteWord(unsigned int a, unsigned short d)
{
	if (a >= 0x160800 && a <= 0x160807) return;
...
unsigned short __fastcall Jumppop68KReadWord(unsigned int a)
{
	switch (a) {
		case 0x180002: {
			return ((0xff - DrvInput[1]) << 8) | (0xff - DrvInput[0]);
...
void __fastcall Jumppop68KWriteWord(unsigned int a, unsigned short d)
{
...
unsigned char __fastcall JumpkidsZ80Read(unsigned short a)
{
	switch (a) {
		case 0x9800: {
			return MSM6295ReadStatus(0);
...
void __fastcall JumpkidsZ80Write(unsigned short a, unsigned char d)
{
	switch (a) {
		case 0x9000: {
			DrvOkiBank = d & 3;
...
unsigned char __fastcall SemicomZ80Read(unsigned short a)
{
	switch (a) {
		case 0xf000:
		case 0xf001: {
			return BurnYM2151ReadStatus(a & 0x01);
		}
...
void __fastcall SemicomZ80Write(unsigned short a, unsigned char d)
{
	switch (a) {
		case 0xf000: {
			BurnYM2151SelectRegister(d);
...
unsigned char __fastcall JumppopZ80PortRead(unsigned short a)
{
	a &= 0xff;
...
void __fastcall JumppopZ80PortWrite(unsigned short a, unsigned char d)
{
	a &= 0xff;
...
static int ALIGN_DATA CharPlaneOffsets[4]     = { 0x200008, 0x200000, 8, 0 };
static int ALIGN_DATA CharXOffsets[8]         = { 0, 1, 2, 3, 4, 5, 6, 7 };
static int ALIGN_DATA CharYOffsets[8]         = { 0, 16, 32, 48, 64, 80, 96, 112 };
static int ALIGN_DATA SpritePlaneOffsets[4]   = { 0x400008, 0x400000, 8, 0 };
static int ALIGN_DATA Sprite2PlaneOffsets[4]  = { 0x800008, 0x800000, 8, 0 };
static int ALIGN_DATA Sprite3PlaneOffsets[4]  = { 0x1000008, 0x1000000, 8, 0 };
static int ALIGN_DATA SpriteXOffsets[16]      = { 256, 257, 258, 259, 260, 261, 262, 263, 0, 1, 2, 3, 4, 5, 6, 7 };
static int ALIGN_DATA SpriteYOffsets[16]      = { 0, 16, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240 };
static int ALIGN_DATA SuprtrioPlaneOffsets[4] = { 0x400000, 0, 0x600000, 0x200000 };
static int ALIGN_DATA SuprtrioXOffsets[16]    = { 0, 1, 2, 3, 4, 5, 6, 7, 128, 129, 130, 131, 132, 133, 134, 135 };
static int ALIGN_DATA SuprtrioYOffsets[16]    = { 8, 0, 16, 24, 40, 32, 48, 56, 64, 72, 80, 88, 96, 104, 112, 120 };
static int ALIGN_DATA JpCharPlaneOffsets[8]   = { 0, 1, 2, 3, 4, 5, 6, 7 };
static int ALIGN_DATA JpCharXOffsets[8]       = { 0x800000, 0x800008, 0, 8, 0x800010, 0x800018, 16, 24 };
static int ALIGN_DATA JpCharYOffsets[8]       = { 0, 32, 64, 96, 128, 160, 192, 224 };
static int ALIGN_DATA JpTilePlaneOffsets[8]   = { 0, 1, 2, 3, 4, 5, 6, 7 };
static int ALIGN_DATA JpTileXOffsets[16]      = { 0x800000, 0x800008, 0, 8, 0x800010, 0x800018, 16, 24, 0x800100, 0x800108, 256, 264, 0x800110, 0x800118, 272, 280 };
static int ALIGN_DATA JpTileYOffsets[16]      = { 0, 32, 64, 96, 128, 160, 192, 224, 512, 544, 576, 608, 640, 672, 704, 736 };

static void TumblebTilesRearrange()
{
	UINT8 *rom = DrvTempRom;
	int len = DrvNumTiles * 128;
	int i;
...
static int TumblebLoadRoms()
{
	...
	DecodeGfxRom(DrvNumTiles, 4, 16, 16, CharPlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvTiles);
	...
	DecodeGfxRom(DrvNumSprites, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);
...
static int MetlsavrLoadRoms()
{
	...
	DecodeGfxRom(DrvNumSprites, 4, 16, 16, Sprite2PlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);
...
static int PangpangLoadRoms()
{
	...
	DecodeGfxRom(DrvNumTiles, 4, 16, 16, SpritePlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvTiles);
...
static int SuprtrioLoadRoms()
{
	...
	DecodeGfxRom(DrvNumTiles, 4, 16, 16, SuprtrioPlaneOffsets, SuprtrioXOffsets, SuprtrioYOffsets, 0x100, DrvTempRom, DrvTiles);
...
static int HtchctchLoadRoms()
{
	...
	DecodeGfxRom(DrvNumSprites, 4, 16, 16, CharPlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);
...
static int ChokchokLoadRoms()
{
	...
	DecodeGfxRom(DrvNumTiles, 4, 16, 16, Sprite2PlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvTiles);
...
static int SdfightLoadRoms()
{
	...
	DecodeGfxRom(DrvNumSprites, 4, 16, 16, Sprite3PlaneOffsets, SpriteXOffsets, SpriteYOffsets, 0x200, DrvTempRom, DrvSprites);
...
static int DrvInit(bool bReset, int SpriteRamSize, int SpriteMask, int SpriteXOffset, int SpriteYOffset, int NumSprites, int NumChars, int NumTiles, float Refresh, int OkiFreq)
{
	int nRet = 0, nLen;
	...
	if (DrvHasYM2151) {
		...
		BurnYM2151Init(DrvYM2151Freq, 10.0);
...
static int Tumbleb2Init()
{
	Tumbleb2 = 1;
	return TumblebInit();	
}

static int JumpkidsInit()
{
	int nRet;
...
static int CookbibInit()
{
	int nRet = HtchctchInit();
...
static int JumppopInit()
{
	...
	DecodeGfxRom(DrvNumTiles, 8, 16, 16, JpTilePlaneOffsets, JpTileXOffsets, JpTileYOffsets, 0x400, DrvTempRom, DrvTiles);
...
inline static unsigned int HtchctchCalcCol(unsigned short nColour)
{
	int r, g, b;
...
inline static unsigned int FncywldCalcCol(unsigned short nColour)
{
	int r, g, b;
...
inline static unsigned int JumppopCalcCol(unsigned short nColour)
{
	int r, g, b;
...
static void DrvRenderPf2Layer(int ScrollX, int ScrollY)
{
...
static void DrvRenderPf1Layer(int ScrollX, int ScrollY)
{
	int mx, my, Attr, Code, Colour, x, y, TileIndex;
...
static void DrvRenderSprites(int MaskColour, int xFlipped)
{
	int Offset;
	UINT16 *SpriteRam = (UINT16*)DrvSpriteRam;
	
	for (Offset = 0; Offset < DrvSpriteRamSize / 2; Offset += 4) {
		int x, y, Code, Colour, Flash, Multi, xFlip, yFlip, Inc, Mult;
		...
		while (Multi >= 0) {
			int RenderCode = Code - (Multi * Inc);
			int RenderX = DrvSpriteXOffset + x;
			int RenderY = DrvSpriteYOffset + y + (Mult * Multi);
...
static int JumppopFrame()
{
	int nInterleave = 1953;
