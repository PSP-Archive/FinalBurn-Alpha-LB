...
static unsigned int ALIGN_DATA VidRegs[5] = {0,0,0,0,0};
...
static int Scroll2Y;
static int SndBank;
static int ALIGN_DATA nCyclesTotal[1];

static unsigned char  ALIGN_DATA BiomtoyInputPort0[8] = {0, 0, 0, 0, 0, 0, 0, 0};
static unsigned char  ALIGN_DATA BiomtoyInputPort1[8] = {0, 0, 0, 0, 0, 0, 0, 0};
static unsigned char  ALIGN_DATA BiomtoyDip[2]        = {0, 0};
static unsigned short ALIGN_DATA BiomtoyInput[2]      = {0x00,0x00};
...
static g_sprite* ALIGN_DATA pSpriteList[5] = {NULL,NULL,NULL,NULL,NULL};
static int ALIGN_DATA spritelistcount[5] = {0,0,0,0,0};
...
int BiomtoyCalcPalette()
{
	...
	for (i = 0, ps = (unsigned short*)PaletteRam, pd = LocalPalette; i < 0x400 / 16; i++) {
		*pd++ = CalcCol(*ps++);
		*pd++ = CalcCol(*ps++);
		*pd++ = CalcCol(*ps++);
		*pd++ = CalcCol(*ps++);
...
static void spritetest(int Number, int Startx, int Starty, int Colour, bool flipx,bool flipy)
{
...
static void tiletest(int Number, int Startx, int Starty, int Colour,bool flipx)
{
	int x, y, tempCol, sx, sy;
	unsigned char *temp;
...
//	nCyclesTotal[0] = (int)((long long)(24000000 / 2) * nBurnCPUSpeedAdjust / (0x0100 * 60));
	nCyclesTotal[0] = (int)((long long)(24000000 / 2) / 60);
...
struct BurnDriverD BurnDrvManiacsp = {
