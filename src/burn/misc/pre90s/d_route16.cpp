...
static unsigned char ALIGN_DATA DrvJoy1[24], DrvJoy2[8], Dips, DrvReset;
...
static int DrvInit()
{
	...
	memset(Mem, 0, (0x10000 + 0x10000 + 0x200));
...
inline static unsigned int route16_make_pen(unsigned char color)
{
	unsigned int ret = 0;
...
inline static unsigned int ttmajng_make_pen(unsigned char color)
{
	unsigned int ret = 0;
...
#ifdef BUILD_PSP
			if (flipscreen)
				PutPix(pBurnDraw + ((x << 9) | (y ^ 0xff)) * nBurnBpp, BurnHighCol(pen>>16, pen>>8, pen, 0));
			else
				PutPix(pBurnDraw + (((x ^ 0xff) << 9) | y) * nBurnBpp, BurnHighCol(pen>>16, pen>>8, pen, 0));
#else
			if (flipscreen)
