...
static unsigned char ALIGN_DATA DrvJoy[8], DrvReset, *Mem, nColor;
...
#ifdef BUILD_PSP
			PutPix(pBurnDraw + ((x + 0) << 1) + ((y + 0) << 10), color());
			PutPix(pBurnDraw + ((x + 1) << 1) + ((y + 0) << 10), color());
			PutPix(pBurnDraw + ((x + 0) << 1) + ((y + 1) << 10), color());
			PutPix(pBurnDraw + ((x + 1) << 1) + ((y + 1) << 10), color());
#else
