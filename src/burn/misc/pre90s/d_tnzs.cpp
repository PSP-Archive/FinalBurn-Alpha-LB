...
static int tnzs_gfx_convert()
{
	...
	DecodeGfxRom(0x2000, 4, 16, 16, Plane, XOffs, YOffs, 0x100, tmp, Gfx);
...
static int DrvInit()
{
	...
	memset(Mem, 0, (0x30000 + 0x20000 + 0x10000 + 0x200000 + 0x804));
	...
	BurnYM2203Init(1, 3000000, 15.0, 35.0, &DrvYM2203IRQHandler, DrvSynchroniseStream, DrvGetTime, 0);
...
inline static void tnzs_plotpix(int x, unsigned char y, int color, unsigned char src, int transp)
{
...
static void clear_background(int col)
{
	...
	int pxl = Palette[col];
	clear_gui_texture( BurnHighCol(pxl >> 16, pxl >> 8, pxl, 0), 256, 256 );
#else
