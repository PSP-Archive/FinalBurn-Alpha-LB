...
static unsigned char ALIGN_DATA DrvJoy1[6], DrvJoy2[6], DrvReset, DrvDips;
static short *pAY8910Buffer[3], *pFMBuffer = NULL;
static unsigned int ALIGN_DATA Palette[8];
...
static int DrvInit()
{
	...
	AY8910Init(0, (8000000 / 4), nBurnSoundRate, NULL, NULL, NULL, NULL);
...
#define cl(v)	(v & 0x80) ? 0xff : 0
	...
	for (int i = 0x80; i < 0x800 - 0x80; i+=2)
	{
		...
		for (int y = y1; y < y1 + 256 * 8; y+=256, tile_no++)
		{
			...
			{
#ifdef BUILD_PSP
				PutPix(pBurnDraw + ((y << 2) | (x << 1)), BurnHighCol(cl(r0), cl(g0), cl(b0), 0));
#else
