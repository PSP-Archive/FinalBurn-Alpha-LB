...
static unsigned char ALIGN_DATA DrvDips[3];
static unsigned char ALIGN_DATA DrvInputs[3];
static unsigned char DrvReset;

static int ALIGN_DATA nCyclesTotal[2];
...
{
	int ALIGN_DATA Plane[4] = { ((0x40000 * 8) / 2) + 4, ((0x40000 * 8) / 2) + 0, 4, 0 };
	int ALIGN_DATA XOffs[16] = { 0, 1, 2, 3, 8+0, 8+1, 8+2, 8+3,
			16*16+0, 16*16+1, 16*16+2, 16*16+3, 16*16+8+0, 16*16+8+1, 16*16+8+2, 16*16+8+3 };
	int ALIGN_DATA YOffs[16] = { 0*16, 1*16, 2*16, 3*16, 4*16, 5*16, 6*16, 7*16,
...
//	ZetMapArea(0xd800, 0xdfff, 1, DrvPalRAM);
	...
	BurnYM2203Init(2, 3579545, 15.0, 15.0, &DrvFMIRQHandler, DrvSynchroniseStream, DrvGetTime, 0);
...
static struct BurnRomInfo bktigerbRomDesc[] = {
...
STD_ROM_PICK(bktigerb);
STD_ROM_FN(bktigerb);

struct BurnDriver BurnDrvBktigerb = {
	"bktigrb1", "blktiger", NULL, "1987",
	...
	NULL, bktigerbRomInfo, bktigerbRomName, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, 0, NULL, NULL, NULL, &DrvRecalc,
	256, 224, 4, 3
};
...
static struct BurnRomInfo bktigercRomDesc[] = {
	{ "1.bin",		0x08000, 0x47E2B21E, 1 | BRF_PRG | BRF_ESS }, //  0 - Z80 #0 Code
	{ "2.bin",		0x10000, 0x7bef96e8, 1 | BRF_PRG | BRF_ESS }, //  1
	{ "3.bin",		0x10000, 0x52c56ed1, 1 | BRF_PRG | BRF_ESS }, //  2
	{ "4.bin",		0x10000, 0xed6af6ec, 1 | BRF_PRG | BRF_ESS }, //  3
	{ "5.bin",		0x10000, 0xae59b72e, 1 | BRF_PRG | BRF_ESS }, //  4

	{ "6.bin",		0x08000, 0x2cf54274, 2 | BRF_PRG | BRF_ESS }, //  5 - Z80 #0 Code

	{ "15.bin",		0x08000, 0x70175d78, 3 | BRF_GRA },           //  6 - Characters

	{ "12.bin",		0x10000, 0xc4524993, 4 | BRF_GRA },           //  7 - Background Tiles
	{ "11.bin",		0x10000, 0x7932c86f, 4 | BRF_GRA },           //  8
	{ "14.bin",		0x10000, 0xdc49593a, 4 | BRF_GRA },           //  9
	{ "13.bin",		0x10000, 0x7ed7a122, 4 | BRF_GRA },           // 10

	{ "8.bin",		0x10000, 0xe2f17438, 5 | BRF_GRA },           // 11 - Sprites
	{ "7.bin",		0x10000, 0x5fccbd27, 5 | BRF_GRA },           // 12
	{ "10.bin",		0x10000, 0xfc33ccc6, 5 | BRF_GRA },           // 13
	{ "9.bin",		0x10000, 0xf449de01, 5 | BRF_GRA },           // 14
};

STD_ROM_PICK(bktigerc);
STD_ROM_FN(bktigerc);

struct BurnDriver BurnDrvbktigerc = {
	"bktigerc", "blktiger", NULL, "1987",
	"Black Tiger (bootleg set 2)\0", NULL, "bootleg", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_PRE90S, GBF_PLATFORM | GBF_SCRFIGHT, 0,
	NULL, bktigercRomInfo, bktigercRomName, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, 0, NULL, NULL, NULL, &DrvRecalc,
	256, 224, 4, 3
};
*/
...
static struct BurnRomInfo blkdrgnbRomDesc[] = {
...
STD_ROM_PICK(blkdrgnb);
STD_ROM_FN(blkdrgnb);

struct BurnDriver BurnDrvBlkdrgnb = {
	"blkdrgnb", "blktiger", NULL, "1987",
	"Black Dragon (bootleg)\0", NULL, "bootleg", "Miscellaneous",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE, 2, HARDWARE_MISC_PRE90S, GBF_PLATFORM | GBF_SCRFIGHT, 0,
	NULL, blkdrgnbRomInfo, blkdrgnbRomName, DrvInputInfo, DrvDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, 0, NULL, NULL, NULL, &DrvRecalc,
	256, 224, 4, 3
};
