...
int GalNoiseVolume;
float GalShootWavePos;
float GalNoiseWavePos;
float GalLfoWavePos[3];
int GalPitch;
int GalVol;
static float GalCounter;
static int GalCountDown;
int GalLfoVolume[3];
float GalLfoFreq;
float GalLfoFreqFrameVar;
int GalLfoBit[4];
...
void GalSoundInit()
{
	...
	if (GalSoundType == GAL_SOUND_HARDWARE_TYPE_GALAXIAN || GalSoundType == GAL_SOUND_HARDWARE_TYPE_KINGBALLDAC) {
		...
		float v  = 5.0;
		float vK = (GalShootRate) ? exp(-1 / (R41__ * C25__) / GalShootRate) : 0;
		float IC8L3 = IC8L3_L;
		int IC8Lcnt = (int)(SHOOT_KEYON_TIME * GalShootRate);
		float c28v = IC8L3_H - (IC8L3_H - (NOISE_H + NOISE_L) / 2) / (R46__ + R47__ + R48__) * R47__;
		float c28K = (GalShootRate) ? exp(-1 / (22000 * 0.000047 ) / GalShootRate) : 0;
		float c29v  = IC8L3_H - (IC8L3_H - (NOISE_H + NOISE_L) / 2) / (R46__ + R47__ + R48__) * (R47__ + R48__);
		float c29K1 = (GalShootRate) ? exp(-1 / (22000  * 0.00000001 ) / GalShootRate) : 0;
		float c29K2 = (GalShootRate) ? exp(-1 / (100000 * 0.00000001 ) / GalShootRate) : 0;
		float ne555cnt = 0;
		float ne555step = (GalShootRate) ? ((1.44 / ((R44__ + R45__ * 2) * C27__)) / GalShootRate) : 0;
		float ne555duty = (float)(R44__ + R45__) / (R44__ + R45__ * 2);
		float ne555sr;
		float ncnt  = 0.0;
		float nstep = (GalShootRate) ? ((float)NOISE_RATE / GalShootRate) : 0;
		float noise_sh2;
...
{
	float Addr = GalShootWavePos;
	float Step = (float)GalShootRate / nBurnSoundRate;
...
{
	float Addr = GalNoiseWavePos;
	float Step = (float)NOISE_RATE / nBurnSoundRate;
...
static void GalRenderToneWave(short *pSoundBuf, int nLength)
{
	...
	if (GalPitch != 0xff) {
		for (i = 0; i < nLength; i += 2) {
			...
			for (j = 0; j < STEPS; j++) {
				if (GalCountDown >= 256) {
					GalCounter = GalCounter + ((float)96000 / nBurnSoundRate);
...
{
	float Addr = GalLfoWavePos[nLfoWave];
	float Step = (float)(sizeof(GalBackgroundWave) * GalLfoFreq * (100 + 2 * 470) / (100 + 2 * 470)) / nBurnSoundRate;
