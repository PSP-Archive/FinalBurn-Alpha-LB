...
static float ComputeResistorWeights(int MinVal, int MaxVal, float Scaler, int Count1, const int *Resistances1, float *Weights1, int PullDown1, int PullUp1,	int Count2, const int *Resistances2, float *Weights2, int PullDown2, int PullUp2, int Count3, const int *Resistances3, float *Weights3, int PullDown3, int PullUp3)
{
	int NetworksNum;

	int ResCount[MAX_NETS];
	float r[MAX_NETS][MAX_RES_PER_NET];
	float w[MAX_NETS][MAX_RES_PER_NET];
	float ws[MAX_NETS][MAX_RES_PER_NET];
	int r_pd[MAX_NETS];
	int r_pu[MAX_NETS];

	float MaxOut[MAX_NETS];
	float *Out[MAX_NETS];
...
void GalaxianCalcPalette()
{
	...
	float rWeights[3], gWeights[3], bWeights[2];
...
// Bullet rendering
inline static void GalDrawPixel(int x, int y, int Colour)
{
