...
int ALIGN_DATA nSekCPUType[SEK_MAX], nSekCycles[SEK_MAX], nSekIRQPending[SEK_MAX];
...
static int SekInitCPUC68K(int nCount, int nCPUType)
{
	...
	SekC68KContext[nCount] = (c68k_struc *)memalign(4, sizeof(c68k_struc));
...
int SekInit(int nCount, int nCPUType)
{
	...
	SekExt[nCount] = (struct SekExt*)memalign(4, sizeof(struct SekExt));
