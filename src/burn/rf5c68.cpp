...
#else
		left[i] = __builtin_allegrex_min(left[i],  32767);
		left[i] = __builtin_allegrex_max(left[i], -32768);
		left[i] = left[i] & ~0x3f;
		right[i] = __builtin_allegrex_min(right[i],  32767);
		right[i] = __builtin_allegrex_max(right[i], -32768);
		right[i] = right[i] & ~0x3f;
#endif
...
void RF5C68PCMInit(int clock)
{
	...
	memset(chip, 0, sizeof(struct rf5c68pcm));
	...
	memset(left, 0, nBurnSoundRate * sizeof(int));
	memset(right, 0, nBurnSoundRate * sizeof(int));
}
