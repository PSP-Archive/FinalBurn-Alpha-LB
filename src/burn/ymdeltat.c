...
const  INT8 ALIGN_DATA ym_deltat_decode_tableB1[16] = {
  1,   3,   5,   7,   9,  11,  13,  15,
 -1,  -3,  -5,  -7,  -9, -11, -13, -15,
};
/* delta to next delta (rate= *64) */
/* 0.9 , 0.9 , 0.9 , 0.9 , 1.2 , 1.6 , 2.0 , 2.4 */
const UINT8 ALIGN_DATA ym_deltat_decode_tableB2[16] = {
...
static const UINT8 ALIGN_DATA dram_rightshift[4] = { 3,0,0,0 };
		...
		if ((DELTAT->control2 & 3) != (v & 3)) {
			...
			if (DELTAT->DRAMportshift != dram_rightshift[v & 3]) {
				...
				*/
				...
				/* refresh addresses */
				DELTAT->start  = ((DELTAT->reg[0x3] << 8) | DELTAT->reg[0x2]) << (DELTAT->portshift - DELTAT->DRAMportshift);
				DELTAT->end    = ((DELTAT->reg[0x5] << 8) | DELTAT->reg[0x4]) << (DELTAT->portshift - DELTAT->DRAMportshift);
				DELTAT->end   += ((1 << (DELTAT->portshift-DELTAT->DRAMportshift)) - 1);
				DELTAT->limit  = ((DELTAT->reg[0xd] << 8) | DELTAT->reg[0xc]) << (DELTAT->portshift - DELTAT->DRAMportshift);
			}
		}
		DELTAT->control2 = v;
		break;
...
*/
	...
	*/
	  ...
	  break;
		...
		case 0x0a:	/* DELTA-N H */
		DELTAT->delta  = ((DELTAT->reg[0xa] << 8) | DELTAT->reg[0x9]);
		...
		case 0x0b:	/* Output level control (volume, linear) */
		{
			...
			DELTAT->volume = (v & 0xff) * (DELTAT->output_range >> 8) / YM_DELTAT_DECODE_RANGE;
/*								v	  *		((1<<16)>>8)		>>	15;
*						thus:	v	  *		(1<<8)				>>	15;
...
INLINE void YM_DELTAT_synthesis_from_external_memory(YM_DELTAT *DELTAT)
{
	...
	if (DELTAT->now_step >= (1 << YM_DELTAT_SHIFT)) {
		...
		do {
			...
			DELTAT->acc += ((ym_deltat_decode_tableB1[data] * DELTAT->adpcmd) >> 3);
			YM_DELTAT_Limit(DELTAT->acc,YM_DELTAT_DECODE_MAX, YM_DELTAT_DECODE_MIN);

			/* delta to next delta */
			DELTAT->adpcmd = (DELTAT->adpcmd * ym_deltat_decode_tableB2[data]) >> 6;
