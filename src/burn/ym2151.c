...
typedef struct {
	...
	UINT8		mul;					/* frequency count multiply */
	UINT8		dt1_i;					/* DT1 index * 32 */
	UINT16		dt2;					/* current DT2 (detune 2) value */
	...
	UINT16		fb_shift;				/* feedback shift value for operators 0 in each channel */
	...
	UINT16		kc;						/* channel KC (copied to all operators) */
	UINT16		kc_i;					/* just for speedup */
	UINT8		pms;					/* channel PMS */
	...
	UINT16		tl;						/* Total attenuation Level */
	INT16		volume;					/* current envelope attenuation level */
	UINT8		eg_sh_d1r;				/*  (decay state) */
	UINT8		eg_sel_d1r;				/*  (decay state) */
	UINT8		d1l;					/* envelope switches to sustain state after reaching this level */
	UINT8		eg_sh_d2r;				/*  (sustain state) */
	UINT8		eg_sel_d2r;				/*  (sustain state) */
	UINT8		eg_sh_rr;				/*  (release state) */
	UINT8		eg_sel_rr;				/*  (release state) */

	UINT8		key;					/* 0=last key was KEY OFF, 1=last key was KEY ON */

	UINT8		ks;						/* key scale    */
	UINT8		ar;						/* attack rate  */
	UINT8		d1r;					/* decay rate   */
	UINT8		d2r;					/* sustain rate */
	UINT8		rr;						/* release rate */
...
typedef struct {
	...
	UINT16		lfo_phase;				/* accumulated LFO phase (0 to 255) */
	...
	UINT16		lfo_counter;			/* LFO phase increment counter		*/
	UINT8		lfo_counter_add;		/* step of lfo_counter				*/
	...
	UINT16		lfa;					/* LFO current AM output			*/
	INT16		lfp;					/* LFO current PM output			*/

	UINT8		test;					/* TEST register */
	UINT8		ct;						/* output control pins (bit1-CT2, bit0-CT1) */

	UINT8		noise;					/* noise enable/period register (bit 7 - noise enable, bits 4-0 - noise period */
	UINT32		noise_rng;				/* 17 bit noise shift register */
	UINT32		noise_p;				/* current noise 'phase'*/
	UINT32		noise_f;				/* current noise period */

	UINT8		csm_req;				/* CSM  KEY ON / KEY OFF sequence request */

	UINT8		irq_enable;				/* IRQ enable for timer B (bit 3) and timer A (bit 2); bit 7 - CSM mode (keyon to all slots, everytime timer A overflows) */
	UINT8		status;					/* chip status (BUSY, IRQ Flags) */
...
static INT16 ALIGN_DATA tl_tab[TL_TAB_LEN];
...
static UINT8 ALIGN_DATA d1l_tab[16];
...
*	0     600   781   950
*/
static const UINT16 ALIGN_DATA dt2_tab[4] = { 0, 384, 500, 608 };
...
static UINT8 YMNumChips;		/* total # of YM2151's emulated */
...
static void init_tables(void)
{
	...
	for (x = 0; x < TL_RES_LEN; x++) {
		...
		tl_tab[(x << 1) + 0] = n;
		tl_tab[(x << 1) + 1] = -tl_tab[(x << 1) + 0];

		for (i = 1; i < 13; i++) {
			tl_tab[(x << 1) + 0 + (i << 1) * TL_RES_LEN] =  tl_tab[(x << 1) + 0] >> i;
			tl_tab[(x << 1) + 1 + (i << 1) * TL_RES_LEN] = -tl_tab[(x << 1) + 0 + (i << 1) * TL_RES_LEN];
		}
	}

	for (i = 0; i < SIN_LEN; i++) {
		/* non-standard sinus */
		m = sinf(((i << 1) + 1) * PI / SIN_LEN); /* verified on the real chip */

		/* we never reach zero here due to ((i*2)+1) */

		if (m > 0.0)
			o = 8 * logf( 1.0 / m) / logf(2.0);	/* convert to 'decibels' */
		else
			o = 8 * logf(-1.0 / m) / logf(2.0);	/* convert to 'decibels' */
...
static void init_chip_tables(YM2151 *chip)
{
	...
	for (j = 0; j < 4; j++) {
		for (i = 0; i < 32; i++) {
			Hz = ((float)dt1_tab[(j << 5) + i] * ((float)chip->clock / 64.0)) / (float)(1 << 20);

			/*calculate phase increment*/
			phaseinc = (Hz * SIN_LEN) / (float)chip->sampfreq;

			/*positive and negative values*/
			chip->dt1_freq[((j + 0) << 5) + i] = phaseinc * mult;
			chip->dt1_freq[((j + 4) << 5) + i] = -chip->dt1_freq[((j + 0) << 5) + i];
		}
	}
	...
	for (i = 0; i < 32; i++) {
		...
		j = 32 - j;
		j = (65536.0 / (float)(j << 5));	/* number of samples per one shift of the shift register */
		/*chip->noise_tab[i] = j * 64;*/	/* number of chip clock cycles per one shift */
		chip->noise_tab[i] = (j << 6) * scaler;
...
{																						\
	if (!(op)->key) {																	\
		...
		int incr = eg_inc[(op)->eg_sel_ar + ((PSG->eg_cnt >> (op)->eg_sh_ar) & 7)];		\
...
static void envelope_KONKOFF(YM2151Operator *op, UINT8 v)
{
	if (v & 0x08)	/* M1 */
		KEY_ON (op + 0, 1)
	else
...
static void timer_callback_a (int n)
{
	...
	if (chip->irq_enable & 0x04) {
		UINT8 old_state = chip->status & 3;
		chip->status |= 1;

		if ((!old_state) && (chip->irqhandler)) (*chip->irqhandler)(1);
	}
...
static void set_connect(YM2151Operator *om1, UINT8 ch, UINT8 v)
{
...
void YM2151WriteReg(UINT8 n, UINT8 r, UINT8 v)
{
	YM2151 *chip = &YMPSG[n];
	YM2151Operator *op = &chip->oper[((r & 0x07) << 2) + ((r & 0x18) >> 3)];
	...
	switch (r & 0xe0) {
		...
		switch (r) {
			...
			envelope_KONKOFF(&chip->oper[(v & 7) << 2], v);
			break;
			...
			if (v & 0x20) {		/* reset timer B irq flag */
				...
				if ((old_state == 2) && (chip->irqhandler)) (*chip->irqhandler)(0);
			}
			...
			if (v & 0x10) {		/* reset timer A irq flag */
				...
				if ((old_state == 1) && (chip->irqhandler)) (*chip->irqhandler)(0);
			}
...
//			logerror("YM2151 Write %02x to undocumented register #%02x\n",v,r);
		...
		switch (r & 0x18) {
			...
			chip->pan[((r & 7) << 1) + 0] = (v & 0x40) ? ~0 : 0;
			chip->pan[((r & 7) << 1) + 1] = (v & 0x80) ? ~0 : 0;
			...
			if ((UINT16)v != op->kc) {
				...
				for (i = 0; i < 4; i++) {
					...
					(op + i)->dt1  = chip->dt1_freq[(op + i)->dt1_i + kc];
					(op + i)->freq = ((chip->freq[kc_channel + (op + i)->dt2] + (op + i)->dt1) * (op + i)->mul) >> 1;
					refresh_EG(op + i);
				}
			}
			break;
		...
		case 0x40:		/* DT1, MUL */
		{
			UINT8 old_dt1_i = op->dt1_i;
			UINT8 old_mul = op->mul;

			op->dt1_i = (v & 0x70) << 1;
			op->mul   = (v & 0x0f) ? ((v & 0x0f) << 1) : 1;

			if (old_dt1_i != op->dt1_i)
				op->dt1 = chip->dt1_freq[op->dt1_i + (op->kc >> 2)];

			if ((old_dt1_i != op->dt1_i) || (old_mul != op->mul))
...
UINT8 YM2151ReadStatus( UINT8 n )
{
...
static void ym2151_state_save_register( int numchips )
{
	...
	for (i = 0; i < numchips; i++) {
		...
		for (j = 0; j < 32; j++) {
			...
			state_save_register_UINT8  (buf1, i, "mul"  , &op->mul,   1);
			state_save_register_UINT8  (buf1, i, "dt1_i", &op->dt1_i, 1);
			state_save_register_UINT16 (buf1, i, "dt2"  , &op->dt2,   1);
			/* operators connection is saved in chip data block */
			state_save_register_INT32  (buf1, i, "mem_v" ,&op->mem_value,1);

			state_save_register_UINT16 (buf1, i, "fb_sh", &op->fb_shift,   1);
			state_save_register_INT32  (buf1, i, "fb_c" , &op->fb_out_curr,1);
			state_save_register_INT32  (buf1, i, "fb_p" , &op->fb_out_prev,1);
			state_save_register_UINT16 (buf1, i, "kc"   , &op->kc,    1);
			state_save_register_UINT16 (buf1, i, "kc_i" , &op->kc_i,  1);
			state_save_register_UINT8  (buf1, i, "pms"  , &op->pms,   1);
			state_save_register_UINT8  (buf1, i, "ams"  , &op->ams,   1);
			state_save_register_UINT32 (buf1, i, "AMmask",&op->AMmask,1);

			state_save_register_UINT8  (buf1, i, "state" ,&op->state,     1);
			state_save_register_UINT8  (buf1, i, "e_shAR",&op->eg_sh_ar,  1);
			state_save_register_UINT8  (buf1, i, "e_slAR",&op->eg_sel_ar, 1);
			state_save_register_UINT16 (buf1, i, "tl"    ,&op->tl,        1);
			state_save_register_INT16  (buf1, i, "volume",&op->volume,    1);
			state_save_register_UINT8  (buf1, i, "e_shD1",&op->eg_sh_d1r, 1);
			state_save_register_UINT8  (buf1, i, "e_slD1",&op->eg_sel_d1r,1);
			state_save_register_UINT8  (buf1, i, "d1l"   ,&op->d1l,       1);
			...
			state_save_register_UINT8  (buf1, i, "key"   ,&op->key, 1);
			state_save_register_UINT8  (buf1, i, "ks"    ,&op->ks,  1);
			state_save_register_UINT8  (buf1, i, "ar"    ,&op->ar,  1);
			state_save_register_UINT8  (buf1, i, "d1r"   ,&op->d1r, 1);
			state_save_register_UINT8  (buf1, i, "d2r"   ,&op->d2r, 1);
			state_save_register_UINT8  (buf1, i, "rr"    ,&op->rr,  1);
		...
		state_save_register_UINT16 (buf1, i, "lfo_phas", &YMPSG[i].lfo_phase,      1);
		...
		state_save_register_UINT16 (buf1, i, "lfo_ctr" , &YMPSG[i].lfo_counter,    1);
		state_save_register_UINT8  (buf1, i, "lfo_ctra", &YMPSG[i].lfo_counter_add,1);
		...
		state_save_register_UINT16 (buf1, i, "lfa"     , &YMPSG[i].lfa, 1);
		state_save_register_INT16  (buf1, i, "lfp"     , &YMPSG[i].lfp, 1);

		state_save_register_UINT8  (buf1, i, "test"    , &YMPSG[i].test,1);
		state_save_register_UINT8  (buf1, i, "ct"      , &YMPSG[i].ct,  1);

		state_save_register_UINT8  (buf1, i, "noise"   , &YMPSG[i].noise,     1);
		...
		state_save_register_UINT8  (buf1, i, "csm_req" , &YMPSG[i].csm_req,   1);
		state_save_register_UINT8  (buf1, i, "irq_ena" , &YMPSG[i].irq_enable,1);
		state_save_register_UINT8  (buf1, i, "status"  , &YMPSG[i].status,    1);
...
*/
char YM2151Init(UINT8 num, int clock, int rate)
{
	UINT8 i;
	...
	if ((YMPSG = (YM2151 *)memalign(4, sizeof(YM2151) * YMNumChips)) == NULL) return 1;
...
*	Reset chip number 'n'.
*/
void YM2151ResetChip(UINT8 num)
{
	short i;
...
#define volume_calc(OP) ((OP)->tl + ((UINT16)(OP)->volume) + (AM & (OP)->AMmask))

INLINE INT16 op_calc(UINT32 phase, UINT32 env, INT32 pm)
{
...
{
	YM2151Operator *op = &PSG->oper[chan << 2];		/* M1 */
	UINT32 AM = ((op->ams) ? (PSG->lfa << (op->ams - 1)) : 0);
	UINT32 env;
...
static void chan7_calc_noise(void)
{
	...
	*op->mem_connect = op->mem_value;	/* restore delayed sample (MEM) value to m2 or c2 */
	...
	env = volume_calc(op + 3);	/* C2 */
	if (env < 0x3ff) {
		env = (env ^ 0x3ff) << 1;	/* range of the YM2151 noise output is -2044 to 2040 */
		chanout[7] += ((PSG->noise_rng & 0x10000) ? env : -env); /* bit 16 -> output */
	}
		...
		do {
			switch(op->state) {
				...
				if (!(PSG->eg_cnt & ((1 << op->eg_sh_ar) - 1))) {
					op->volume += (~op->volume * (eg_inc[op->eg_sel_ar + ((PSG->eg_cnt >> op->eg_sh_ar) & 7)])) >> 4;
	...
	*/
	...
	do {
		if (op->pms) {
			...
			if (mod_ind) {
				UINT16 kc_channel = op->kc_i + mod_ind;
...
#if 0
INLINE INT32 acc_calc(INT32 value)
{
	if (value>=0)
	{
		if (value < 0x0200)
...
*/
void YM2151UpdateOne(UINT8 num, INT16 **buffers, INT16 length)
{
	INT16 i;
	INT32 outl,outr;
	SAMP *bufL, *bufR;
	...
	if (PSG->tim_B) {
		...
		if (PSG->tim_B_val <= 0) {
			...
			if (PSG->irq_enable & 0x08) {
				UINT8 old_state = PSG->status & 3;
				PSG->status |= 2;

				if ((!old_state) && (PSG->irqhandler)) (*PSG->irqhandler)(1);
			}
		}
	}
#endif
...
#else
		outl = __builtin_allegrex_min(outl, MAXOUT);
		outl = __builtin_allegrex_max(outl, MINOUT);

		outr = __builtin_allegrex_min(outr, MAXOUT);
		outr = __builtin_allegrex_max(outr, MINOUT);
#endif
...
void YM2151SetIrqHandler(UINT8 n, void(*handler)(int irq))
{
	YMPSG[n].irqhandler = handler;
}

void YM2151SetPortWriteHandler(UINT8 n, write8_handler handler)
{
