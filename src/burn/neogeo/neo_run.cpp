...
unsigned char ALIGN_DATA NeoButton1[32] = { 0, };
unsigned char ALIGN_DATA NeoButton2[8]  = { 0, };
unsigned char ALIGN_DATA NeoButton3[8]  = { 0, };
unsigned char ALIGN_DATA NeoButton4[8]  = { 0, };
unsigned char ALIGN_DATA NeoJoy1[8]     = { 0, };
unsigned char ALIGN_DATA NeoJoy2[8]     = { 0, };
unsigned char ALIGN_DATA NeoJoy3[8]     = { 0, };
unsigned char ALIGN_DATA NeoJoy4[8]     = { 0, };
unsigned short ALIGN_DATA NeoAxis[2]    = { 0, };
unsigned char ALIGN_DATA NeoInput[32]   = { 0, };
unsigned char ALIGN_DATA NeoDiag[2]     = { 0, };
unsigned char ALIGN_DATA NeoDebugDip[2] = { 0, };
unsigned char NeoReset = 0, NeoSystem = 0;

static unsigned char ALIGN_DATA OldDebugDip[2];

// Which 68K BIOS to use
static int nBIOS;

// Joyports are multiplexed
static int ALIGN_DATA nJoyport0[8] = { 0, };
static int ALIGN_DATA nJoyport1[8] = { 0, };
...
static int ALIGN_DATA nCyclesExtra[2];
...
inline static bool NeoCheckAESBIOS()
{
...
//	NeoSpriteROM = (unsigned char*)malloc(nSpriteSize);
	...
	memset(NeoSpriteROM, 0, nSpriteSize < (nNeoTileMask << 7) ? ((nNeoTileMask + 1) << 7) : nSpriteSize);

	if (BurnDrvGetHardwareCode() & (HARDWARE_SNK_ENCRYPTED_A | HARDWARE_SNK_ENCRYPTED_B)) {
		float fRange = (float)pInfo->nSpriteNum / 4.0;
		if (fRange < 1.5) {
			fRange = 1.5;
		}
	...
	memset(NeoTextROM, 0, nNeoTextROMSize + 0x020000);
	...
	if (pInfo->nADPCMANum) {
		...
		memset(YM2610ADPCMAROM, 0, nYM2610ADPCMASize);
	...
	if (pInfo->nADPCMBNum) {
		...
		memset(YM2610ADPCMBROM, 0, nYM2610ADPCMBSize);
...
inline static void neogeoSynchroniseZ80(int nExtraCycles)
{
#ifdef Z80_SPEED_ADJUST
...
static float neogeoGetTime()
{
	return (float)ZetTotalCycles() / nZ80Clockspeed;
}
...
inline static int NeoConvertIRQPosition(int nOffset)
{
...
inline static void NeoIRQAcknowledge(unsigned short wordValue)
{
...
inline static void SendSoundCommand(const unsigned char nCommand)
{
...
int NeoInit()
{
	...
	{
		unsigned short nNeoScreenHeight; // not used
	...
	BurnYM2610Init(8000000, 60.0, 100.0, YM2610ADPCMAROM, &nYM2610ADPCMASize, YM2610ADPCMBROM, &nYM2610ADPCMBSize, &neogeoFMIRQHandler, neogeoSynchroniseStream, neogeoGetTime, 0);
