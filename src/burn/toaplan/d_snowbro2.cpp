...
static unsigned char ALIGN_DATA DrvInput[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
unsigned char __fastcall snowbro2ReadByte(unsigned int sekAddress)
{
	switch (sekAddress) {
		...
		case 0x500002:
		case 0x500003:
			return BurnYM2151ReadStatus(sekAddress & 0x01);
...
void __fastcall snowbro2WriteWord(unsigned int sekAddress, unsigned short wordValue)
{
	switch (sekAddress) {
		...
		case 0x500000:
			BurnYM2151SelectRegister(wordValue & 0xFF);
			break;
		case 0x500002:
			BurnYM2151WriteRegister(wordValue & 0xFF);
			break;
...
static int DrvInit()
{
	...
	MSM6295Init(0, 27000000 / 10 / 132, 100.0, 1);
