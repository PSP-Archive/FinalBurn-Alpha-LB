...
static const UINT8 ALIGN_DATA bbakraid_unlimited_nvram[512] = {
...
inline static void bbakraidSynchroniseZ80(int nExtraCycles)
{
...
static UINT8 bbakraidTimerOver(UINT8, UINT8)
{
...
//	nCyclesTotal[0] = (int)((long long)16000000 * nBurnCPUSpeedAdjust / (0x0100 * 60));
	...
	unsigned short nSoundBufferPos = 0;
	...
	for (int i = 1; i <= nInterleave; i++) {
		...
		if ((i & 1) == 0) {
			...
			if (pBurnSoundOut) {
				unsigned short nSegmentEnd = nBurnSoundLen * i / nInterleave;
