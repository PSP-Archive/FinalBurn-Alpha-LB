...
static const unsigned char batsugun_cmd_snd[64] = {
...
static struct BurnRomInfo batsugunaRomDesc[] = {
...
STD_ROM_PICK(batsuguna);
STD_ROM_FN(batsuguna);

static struct BurnRomInfo batsugunspRomDesc[] = {
...
STD_ROM_PICK(batsugunsp);
STD_ROM_FN(batsugunsp);
...
void __fastcall batsugunWriteByte(unsigned int sekAddress, unsigned char byteValue)
{
	switch (sekAddress) {
		case 0x21F001:
			...
			else if((byteValue > 0) && (byteValue < 64)) {
				play_oki_sound(batsugun_cmd_snd[byteValue], byteValue);
			}
			break;
...
//	BurnYM2151Init(27000000 / 8, 100.0);
	MSM6295Init(0, 32000000 / 8 / 165, 100.0, 0);
...
struct BurnDriver BurnDrvBatsugunSP = {
	...
	NULL, batsugunspRomInfo, batsugunspRomName, batsugunInputInfo, batsugunDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, 0, NULL, NULL, NULL, &ToaRecalcPalette,
	240, 320, 3, 4
};

struct BurnDriver BurnDrvBatsugna = {
	"batsuguna", "batsugun", NULL, "1993",
	"Batsugun (set 2)\0", "No sound (sound MCU not dumped)", "Dual Toaplan", "Toaplan GP9001 based",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | TOA_ROTATE_GRAPHICS_CCW, 2, HARDWARE_TOAPLAN_68K_Zx80, GBF_VERSHOOT, 0,
	NULL, batsugunaRomInfo, batsugunaRomName, batsugunInputInfo, batsugunDIPInfo,
	DrvInit, DrvExit, DrvFrame, DrvDraw, DrvScan, 0, NULL, NULL, NULL, &ToaRecalcPalette,
	240, 320, 3, 4
};
