...
{
	switch (sekAddress) {
		case 0x500010:								// Coin control
			break;
		
		case 0x500020: {
			RamShared[0] = wordValue & 0xFF;
...
static int drvInit()
{
	...
	MSM6295Init(0, 32000000 / 10 / 132, 100.0, 1);
	MSM6295Init(1, 32000000 / 10 / 165, 100.0, 1);
...
static struct BurnRomInfo batridtaRomDesc[] = {
	...
	{ "prg2.u21",     0x080000, 0xBDAA5FBF, BRF_ESS | BRF_PRG }, //  1
	{ "u23.bin",      0x080000, 0x8ae7f592, BRF_ESS | BRF_PRG }, //  2				(odd)
	{ "prg3.u24",     0x080000, 0x7AA9F941, BRF_ESS | BRF_PRG }, //  3
...
struct BurnDriver BurnDrvBatridta = {
	"batridta", "batrid", NULL, "1998",
	"Armed Police Batrider - A Version (Taiwan) (Mon Dec 22 1997)\0", NULL, "Raizing / 8ing", "Toaplan GP9001 based",
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | TOA_ROTATE_GRAPHICS_CCW, 2, HARDWARE_TOAPLAN_RAIZING, GBF_VERSHOOT, 0,
	NULL, batridtaRomInfo, batridtaRomName, batriderInputInfo, batriderDIPInfo,
	drvInit, drvExit, drvFrame, drvDraw, drvScan, 0, NULL, NULL, NULL, &ToaRecalcPalette,
	240, 320, 3, 4
};
