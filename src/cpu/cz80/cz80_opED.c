...
-----------------------------------------*/
	...
	OPED(0x4f): // LD   R,A
		...
		zR = zA;
#else
		zR = zA - ((CPU->nCyclesSegment - CPU->nCyclesLeft) >> 2);
#endif
		zR2 = zA & 0x80;
		RET(5)
	...
	OPED(0x5f): // LD   A,R
		...
		zA = (zR & 0x7f) | zR2;
#else
		zA = ((zR + ((CPU->nCyclesSegment - CPU->nCyclesLeft) >> 2)) & 0x7f) | zR2;
#endif
		zF = (zF & CF) | SZ[zA] | zIFF2;
		RET(5)
...
-----------------------------------------*/
	...
	OPED(0xb1): // CPIR
		do
		{
			...
			zF = F;
			USE_CYCLES(17)
		} while (zBC && !(F & ZF) && (CPU->nCyclesLeft > 0));
		goto OP_CPXR;
