...
extern unsigned int nRomGameOffset;
...
static const int ALIGN_DATA div_tab[4] = { 3, 5, 3, 0 };
...
INLINE UINT16 OPRW(UINT32 A)
{
	...
	{
		//A &= 0xc7ffffff;
		return *(unsigned short *)getBlockSmallData(nRomGameOffset + (A& 0x00ffffff));
	}
	else
	{
		return *((unsigned short *)(RomBios + (A & SH2_PAGEM)));
	}
}
...
{													\
	...
	else if (sh2->pending_irq & (1 << 14)) irq =14;	\
	else if (sh2->pending_irq & (1 << 13)) irq =13;	\
	else if (sh2->pending_irq & (1 << 12)) irq =12;	\
	else if (sh2->pending_irq & (1 << 11)) irq =11;	\
	else if (sh2->pending_irq & (1 << 10)) irq =10;	\
	else if (sh2->pending_irq & (1 <<  9)) irq =9;	\
	else if (sh2->pending_irq & (1 <<  8)) irq =8;	\
	else if (sh2->pending_irq & (1 <<  7)) irq =7;	\
	else if (sh2->pending_irq & (1 <<  6)) irq =6;	\
	else if (sh2->pending_irq & (1 <<  5)) irq =5;	\
	else if (sh2->pending_irq & (1 <<  4)) irq =4;	\
	else if (sh2->pending_irq & (1 <<  3)) irq =3;	\
	else if (sh2->pending_irq & (1 <<  2)) irq =2;	\
	else if (sh2->pending_irq & (1 <<  1)) irq =1;	\
	else if (sh2->pending_irq & (1 <<  0)) irq =0;	\
