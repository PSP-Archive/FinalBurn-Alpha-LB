...
void ExceptionHandler(PspDebugRegBlock * regs)
{
    ...
    for (;;){
        ...
        if (pad.Buttons & PSP_CTRL_CROSS){
            ...
            if (log != NULL){
                char *trapLog=(char *)malloc(8192);
                char *testo=trapLog;
                testo+=sprintf(testo, "Exception details:\n\n");
                //fwrite(testo, 1, strlen(testo), log);
                testo+=sprintf(testo, "Exception - %s\n", codeTxt[(regs->cause >> 2) & 31]);
                //fwrite(testo, 1, strlen(testo), log);
                testo+=sprintf(testo, "EPC       - %08X / %s.text\n", (int)regs->epc, module_info.modname);//, (unsigned int)(regs->epc-(int)&_ftext));
                //fwrite(testo, 1, strlen(testo), log);
                testo+=sprintf(testo, "Cause     - %08X\n", (int)regs->cause);
                //fwrite(testo, 1, strlen(testo), log);
                testo+=sprintf(testo, "Status    - %08X\n", (int)regs->status);
                //fwrite(testo, 1, strlen(testo), log);
                testo+=sprintf(testo, "BadVAddr  - %08X\n", (int)regs->badvaddr);
                //fwrite(testo, 1, strlen(testo), log);
               	testo+=sprintf(testo, "Used Mem:%d KB, Free Mem:%d KB\n", getUsedMemSize(),getFreeMemSize());
                for(i=0; i<32; i+=4){
                    testo+=sprintf(testo, "%s:%08X %s:%08X %s:%08X %s:%08X\n", regName[i], (int)regs->r[i], regName[i+1], (int)regs->r[i+1], regName[i+2], (int)regs->r[i+2], regName[i+3], (int)regs->r[i+3]);
                    //fwrite(testo, 1, strlen(testo), log);
                }
                fwrite(trapLog,1,testo-trapLog,log);
                fclose(log);
                free(trapLog);
            }
            break;
