...
static int FindRomByName(const TCHAR* szName)
{
	struct ZipEntry* pl;
	int i;
	...
	for (i = 0, pl = List; i < nListCount; i++, pl++) {
		if (strcmp(szName, GetFilenameA(pl->szName)) == 0) {
			return i;
		}
	}
...
static int FindRom(int i)
{
	...
	for (int nAka = 0; nAka < 0x10000; nAka++) {				// Failing that, search for possible names
		const char *szPossibleName = NULL;
...
static int __cdecl BzipBurnLoadRom(unsigned char* Dest, int* pnWrote, int i)
{
	...
	ui_update_progress(ri.nLen ? 1.0 / ((float)nTotalSize / ri.nLen) : 0, szText);
